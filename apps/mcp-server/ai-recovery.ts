...
const reasons: Record<FailureKind, string> = {
  ...
  timeout: 'tempo limite da análise excedido',
...
export class AiRecoveryError extends Error {
  constructor(readonly attempts: Attempt[]) {
    super(`IA não concluída: ${attempts.map(a => `${a.model} (${a.status ? `HTTP ${a.status}, ` : ''}${reasons[a.kind]})`).join('; ')}. Exibindo regras locais, sem revisão por IA.`);
...
export async function withAiRecovery<T>(
  ...
  { models, budgetMs = 45_000, now = Date.now, wait = async (ms, signal) => { await delay(ms, undefined, { signal }); }, log = event => console.error('[ai-review]', JSON.stringify(event)) }: RecoveryOptions,
  ...
  try {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      log({ event: 'attempt', model, attempt: index + 1 });
      try {
        const value = await call(model, controller.signal);
        if (controller.signal.aborted || now() >= deadline) break;
        log({ event: 'success', model, attempt: index + 1 });
        return { value, model };
      } catch (error) {
        ...
        log({ event: 'failure', ...attempts[attempts.length - 1], attempt: index + 1 });
        if (controller.signal.aborted || now() >= deadline) break;
        ...
        if (hasNextModel && ['unavailable', 'model_unavailable'].includes(failure.kind)) modelIndex++;
        if (index === 2) break;
    ...
    if (controller.signal.aborted || now() >= deadline) attempts.push({ model: models[modelIndex], kind: 'timeout' });
    throw new AiRecoveryError(attempts);
