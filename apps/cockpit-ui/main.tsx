...
function Cockpit(){
 ...
 return <main><header><div><span className="eyebrow">AI DEV COCKPIT</span><h1>Revisão com contexto</h1></div><span className="badge">Demonstração</span></header><p className="notice">Dados sintéticos e explicações pré-definidas. Sem consulta ao GitHub ou geração por IA nesta versão.</p>{error&&<p role="alert" className="error">{error}</p>}{!data?<p role="status">{connected?'Peça ao assistente para chamar analyze_pr e abrir o PR de demonstração.':'Aguardando conexão e análise…'}</p>:<><section><span className="eyebrow">{data.repository} · PR #{data.prNumber}</span><h2>{data.title}</h2><p>{data.summary}</p><small>{data.files.length} arquivos · Snapshot {data.headSha}</small></section><div className="layout"><nav aria-label="Arquivos alterados">{data.files.map(f=><button disabled={busy} key={f.path} aria-pressed={selected===f.path} onClick={()=>{setSelected(f.path);setDetail(null);setError('');}}><strong>{f.path}</strong><span>Prioridade {priorities[f.priority]} · +{f.additions} / −{f.deletions}</span></button>)}</nav><section className="file"><h2>{file?.path}</h2><pre aria-label="Trecho alterado"><code>{file?.diff}</code></pre><button className="primary" disabled={busy||!connected||!file} onClick={()=>void explain()}>{busy?'Consultando servidor…':'Explicar alteração'}</button><div aria-live="polite">{detail&&detail.filePath===selected&&<article><h3>O que merece atenção</h3><p>{detail.explanation}</p><h3>O que verificar</h3><p>{detail.check}</p><small>Resposta do servidor: {new Date(detail.requestedAt).toLocaleTimeString('pt-BR')}</small></article>}</div></section></div><footer>A prioridade orienta a revisão humana; não representa garantia de segurança.</footer></>}</main>;
}
